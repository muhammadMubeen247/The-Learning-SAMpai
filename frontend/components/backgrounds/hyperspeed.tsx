...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    class App {
      ...
      update(delta) {
        ...
        let time
        if (this.isPaused) {
          if (this.haltTime == null) {
            this.haltTime = this.clock.elapsedTime + this.timeOffset
          }
          time = this.haltTime
        } else {
          this.haltTime = null
          ...
          this.speedUp += lerp(this.speedUp, this.speedUpTarget, lerpPercentage, 0.00001)
          this.timeOffset += this.speedUp * delta
          time = this.clock.elapsedTime + this.timeOffset
        }
        ...
        const fovChange = lerp(
          this.camera.fov,
          this.fovTarget,
          this.isPaused ? 0 : Math.exp(-(-60 * Math.log2(1 - 0.1)) * delta),
        )
        if (fovChange !== 0) {
  ...
  useEffect(() => {
    if (appRef.current) {
      appRef.current.setPaused(isPaused)
    }
  }, [isPaused])

  useEffect(() => {
    if (appRef.current) {
      appRef.current.setSpeedBoost(speedBoost)
    }
  }, [speedBoost])
